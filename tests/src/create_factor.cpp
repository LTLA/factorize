...
#include "factorize/create_factor.hpp"

template<typename Factor_>
std::pair<std::vector<Factor_>, std::vector<int> > test_create_factor(std::size_t n, const Factor_* factor) {
    std::vector<int> cleand(n, -1); // provide an impossible value to make sure it gets unset.
    auto levels = factorize::create_factor(n, factor, cleand.data());
...
TEST(CleanFactors, Simple) {
    // Simple sorted case.
    {
        ...
        auto cleand = test_create_factor(stuff.size(), stuff.data());
